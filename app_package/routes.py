...
from app_package.forms import LoginForm, RegistrationForm, EditProfileForm, PostForm, ResetPasswordRequestForm, ResetPasswordForm
...
def index():
	...
	if form.validate_on_submit():
		...
		return redirect(url_for('index')) #Why the double re-direct? A standard practice to respond to a POST request generated by a web form submission with a redirect.
...
def register():
	if current_user.is_authenticated:
		return redirect(url_for('index')) #if the user is already logged in, there is no point using the password reset so I redirect to the index
